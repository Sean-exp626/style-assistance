...
[측면 키포인트 — side_keypoints (선택)]
- \`side_keypoints\`는 **측면 사진이 제공된 경우에만** 채우는 선택 필드입니다.
  정면/뒷면만 있다면 \`null\`로 두거나 생략합니다.
- 좌표 규약: **원본 측면 사진 기준 정규화 [0, 1]** 좌표.
  · x = 0 → 사진의 왼쪽 가장자리, x = 1 → 오른쪽 가장자리
  · y = 0 → 사진의 위쪽 가장자리, y = 1 → 아래쪽 가장자리
  · CSS box나 트리밍된 영역이 아니라 **모델에 입력된 원본 픽셀 영역** 기준입니다.
- 7개 앵커 이름은 정확히 다음과 같습니다 (다른 키 사용 금지):
...
- 각 키의 값은 \`{ "x": <number>, "y": <number> }\` 형식이며 두 값 모두 [0, 1] 범위.
- **3개 미만의 키포인트만 자신 있게 짚을 수 있다면 \`side_keypoints\`를 \`null\`로
  반환합니다.** 부분적인 키포인트 객체를 반환하지 마세요 — 자신 있는 부분 집합으로
  채울 거면 최소 3개 이상이어야 합니다.
- 측면이 아닌 사진(정면/뒷면/잘린 사진/매우 흐린 사진)에서도 \`null\`을 반환하세요.
- **자연어 필드(\`face_shape\`, \`head_shape\`, \`professional_analysis\`,
  \`key_features\`)에는 어떤 좌표·숫자도 절대 포함하지 않습니다.** 좌표 정보는
  오직 \`side_keypoints\` 안에서만 표현하세요. 톤 가이드 위반 금지.
...
const OUTPUT_EXAMPLE = `{
  ...
  "side_keypoints": {
    "forehead":    { "x": 0.45, "y": 0.18 },
    "nose_bridge": { "x": 0.32, "y": 0.32 },
    "nose_tip":    { "x": 0.28, "y": 0.42 },
    "philtrum":    { "x": 0.34, "y": 0.55 },
    "lower_lip":   { "x": 0.36, "y": 0.62 },
    "chin":        { "x": 0.42, "y": 0.78 },
    "ear_front":   { "x": 0.65, "y": 0.50 }
  }
}`;
...
- **자연어 필드(face_shape, head_shape, recommended_style.*, key_features, professional_analysis)에는 숫자나 도(°)를 절대 포함하지 않습니다.**`
  ...
  // 측면 사진이 있을 때만 side_keypoints 안내 블록 추가.
  // 클라이언트가 더 이상 MediaPipe 측면 검출을 시도하지 않으므로, 모델이 단일 소스가 된다.
  const sideKeypointsBlock = providedViews.includes("side")
    ? `

[측면 키포인트 출력 지시 — side_keypoints]
- 측면 사진(위 [이미지] 블록 중 \`측면\` 라벨)에서 7개 앵커의 정규화 좌표를 \`side_keypoints\` JSON 필드에 채워 주세요.
- 좌표는 **원본 측면 사진 기준 [0, 1] 정규화 좌표** (x: 좌→우, y: 상→하).
- 7개 앵커: \`forehead\`, \`nose_bridge\`, \`nose_tip\`, \`philtrum\`, \`lower_lip\`, \`chin\`, \`ear_front\`.
- 각 값은 \`{ "x": <0~1>, "y": <0~1> }\`. 자신 있게 짚을 수 있는 키만 채우되,
  **확실한 키포인트가 3개 미만이면 \`side_keypoints\`를 \`null\`로 반환**합니다 (부분 객체 금지).
- 측면이 아니거나 가려진 사진이면 \`null\`을 반환하세요.
- **자연어 필드(face_shape, head_shape, professional_analysis, key_features)에는 어떤 좌표·숫자도 절대 포함하지 않습니다.** 좌표 정보는 오직 \`side_keypoints\` 안에서만.`
    : "";
