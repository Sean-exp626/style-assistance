...
export async function POST(req: Request): Promise<Response> {
  ...
  // V1: 측면 sparse keypoint / 4각도 메트릭은 더 이상 클라이언트에서 보내지 않는다.
  // (Claude Vision이 단일 소스 — `result.side_keypoints` 로 응답에 포함됨)
  // 이전 sideLandmarks/sideMetrics formData 파싱 블록은 V1에서 제거됨.
  ...
  try {
    ...
    try {
      ...
      // V1: sideLandmarks/sideMetrics는 클라이언트가 보내지 않는다 — Firestore에 저장하지 않음.
      // 측면 키포인트는 result.side_keypoints (Claude Vision 응답)로 자동 저장된다.
